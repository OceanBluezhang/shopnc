...
function store_reapply(){
	ajaxget('index.php?act=home&op=store_reapply');
}
</script>
</head>
<body>
...
<div id="header">
  <h1 title="<?php echo C('site_name'); ?>"><a href="<?php echo SiteUrl;?>"><img src="<?php echo C('member_logo') == ''?ATTACH_COMMON.DS.C('site_logo'):ATTACH_COMMON.DS.C('member_logo'); ?>" alt="<?php echo C('site_name'); ?>" class="pngFix"></a><i><?php echo $lang['nc_seller'];?></i></h1>
...
<nav class="navmenu pngFix"><span class="left-side pngFix"></span><span class="right-side pngFix"></span>
  ...
  <?php if($output['store_info']['store_state'] == 0){?>
  <div class="store-close-info"><?php echo $lang['nc_store_state_fail_reason'].($output['store_info']['store_close_info']==''? $lang['nc_store_administrator_not_fill_in']:$output['store_info']['store_close_info']).$lang['nc_period'];?><a href="javascript:void(0);" onclick="if(confirm('<?php echo $lang['nc_store_apply_open_confirm'];?>')){store_reapply();}" ><?php echo $lang['nc_store_apply_open'];?></a></div>
  <?php }elseif($output['store_info']['store_state'] == 2){?>
  <div class="store-close-info"><?php echo $lang['nc_store_administrator_is_audit'];?></div>
  <?php }?>
  <?php }?>
<script type="text/javascript">
// 收缩展开效果
    ...
    <dl>
      ...
      <dd style="display: <?php echo cookie('sidebar_store_manage');?>;">
        <ul>
          ...
          <li><a <?php if($output['menu_sign'] == 'store_sns'){ echo "class='active'";}else{ echo "clas='normal'";}?> href="index.php?act=store_sns"><?php echo $lang['nc_member_path_store_sns'];?></a></li>        </ul>
      </dd>
    </dl>
    <dl>
